...
function layer:_createInitState(batch_size)
  ...
  --self.output_zero = torch.Tensor(batch_size, self.output_size):zero():cuda()
end

function layer:createClones()
  -- construct the net clones
    print('constructing clones inside the attenBlock2')
...
function layer:shareClones()
    ...
    print('resharing clones inside the attenBlock2')
...
function layer:updateOutput(input)
  ...
  self.fore_inputs = {}
  self.core_output:resize(batch_size, self.seq_length, self.output_size):zero()
  ...
  local w_lstm2_ques, w_lstm2_img, ques_atten, img_atten = unpack(self.atten:forward({self.core_output, img, self.mask}))
  --local w_lstm2_ques, w_lstm2_img, ques_atten, img_atten = unpack(self.atten:forward({self.core_output, img, self.mask}))

  return {w_lstm2_ques, w_lstm2_img, ques_atten, img_atten}
  --return {w_lstm2_ques, w_lstm2_img, ques_atten, img_atten}
end
...
function layer:updateGradInput(input, gradOutput)
  ...
  for t=self.tmax,1,-1 do
    ...
    table.insert(dout, d_core_output:narrow(2,t,1):contiguous():view(-1, self.output_size))
