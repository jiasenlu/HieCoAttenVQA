...
function DataLoader:__init(opt)
    ...
    self.MC_ans_test = h5_file:read('/MC_ans_test'):all()

    h5_file:close()
    print('Transform the image feature...')

    --self.fv_im_train = self.fv_im_train:permute(1,4,2,3):contiguous()
    --self.fv_im_test = self.fv_im_test:permute(1,4,2,3):contiguous()
