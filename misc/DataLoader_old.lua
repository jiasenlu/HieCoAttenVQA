...
function DataLoader:__init(opt)
    ...
    print('DataLoader loading h5 image file: ', opt.h5_img_file)
    
    local h5_file = hdf5.open(opt.h5_img_file, 'r')
    ...
    local num_train = self.fv_im_train:size(1)
    local num_test = self.fv_im_test:size(1)


    self.fv_im_train = self.fv_im_train:view(num_train, -1, 512):contiguous()
    self.fv_im_test = self.fv_im_test:view(num_test, -1, 512):contiguous()
    ...
    -- right aligned the questions.
    --self.ques_train = utils.right_align(self.ques_train, self.ques_len_train)
    --.ques_test = utils.right_align(self.ques_test, self.ques_len_test)
...
-- here we use the 5000 sample for the validation 
